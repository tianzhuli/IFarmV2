 ...
 */
@Table
@Entity(name = "farm_controller_multiple_task")
public class MultiControlTask implements Serializable {
	@Id
	...
	@Transient
	List<MultiControlCommand> wfmControlCommads = new ArrayList<MultiControlCommand>(); // 开启按照阀门、罐、水泵的顺序，关闭则相反
	@Transient
	...
	public List<MultiControlCommand> getWfmControlCommands() {
		return wfmControlCommads;
	}

	public void setWfmControlCommads(List<MultiControlCommand> wfmControlCommads) {
	...
	public ControlCommand buildCommand(String type) {
		if ("execution".equals(type)) {
			for (int i = 0; i < wfmControlCommads.size(); i++) {
				MultiControlCommand commad = wfmControlCommads.get(i);
